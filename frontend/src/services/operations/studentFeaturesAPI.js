...
export async function buyCourse (token, courses, userDetails, navigate, dispatch) {
    ...
    if(!orderResponse?.data?.success){
        toast.error(orderResponse?.data?.message || "Unable to initiate payment");
    ...
    const options = {
        ...
        handler: async function (response) {
            ...
            const verified = await verifyPayment(response,courses,token,navigate,dispatch);
            if (verified) {
                await sendPaymentSuccessEmail(response,orderResponse.data.amount,token);
            }
        },
        theme: {
            color: "#686CFD",
...
async function sendPaymentSuccessEmail (response,amount,token) {
    ...
    if (!res?.data?.success) {
        console.log("sendPaymentSuccessEmail -> error", res?.data || res);
        toast.error(res?.data?.message || "Unable to send payment success email");
    }
