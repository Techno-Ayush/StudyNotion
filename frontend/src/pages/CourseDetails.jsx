...
const CourseDetails = () => {
    ...
    useEffect(() => {
        const getCourseDetails = async() => {
            const mockCourse = MOCK_COURSES.find((course) => course._id === courseId);
            if (mockCourse) {
                setCourseDetail(mockCourse);
                return;
            }
    ...
    const instructions = parseInstructions(courseDetail?.instructions);
        ...
        <div className='mx-auto box-content px-4 lg:w-[1260px] lg:relative '>
            ...
            <div className='mx-auto box-content px-4 text-start text-richblack-5 lg:w-[1260px]'>
                ...
                <div className='flex items-center gap-4 py-4'>
                    <img src={courseDetail?.instructor?.image || `https://api.dicebear.com/6.x/initials/svg?seed=${courseDetail?.instructor?.firstName || "Course"} ${courseDetail?.instructor?.lastName || "Author"}`} alt="author img" className='w-[50px] h-[50px] rounded-full object-cover'/>
                    <p className='text-xl font-semibold'>{courseDetail?.instructor?.firstName} {courseDetail?.instructor?.lastName}</p>
                </div>
                <p className='text-richblack-50 text-sm mb-10'>{courseDetail?.instructor?.additionalDetails?.about || "Experienced instructor helping learners build practical coding skills."}</p>
            </div>
